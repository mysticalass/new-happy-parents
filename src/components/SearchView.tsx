...
export const SearchView: React.FC<SearchViewProps> = ({
  ...
  const toggleFavorite = (e: React.MouseEvent, studioId: string) => {
    e.stopPropagation();
    setFavorites((prev) => ({ ...prev, [studioId]: !prev[studioId] }));
  };
        ...
        {filteredStudios.map((studio) => {
          ...
          return (
            <div
              ...
              className="group bg-white rounded-xl border border-[#c3c5d9]/30 overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col cursor-pointer"
            >
              {/* Card Image Header */}
              <div className="relative h-48 w-full overflow-hidden bg-[#e1e2e4]">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 p-2 bg-white/80 backdrop-blur-xs rounded-full text-[#434656] hover:text-red-500 transition-colors shadow-xs"
                >
